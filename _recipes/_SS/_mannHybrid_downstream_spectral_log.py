# -*- coding: utf-8 -*-
"""
Alternative MannHybridTurb downstream calibration recipe.

Method: bounded log-multiplicative spectral update.
This is more theoretically natural for positive spectra, but can be slower or
oscillatory if relaxation is too high. Use it as a comparison against the Wong
style update.
"""
...
cfg = CalibrationConfig(
    method_name="log",
    spectral_relaxation=_env_float("MANN_CAL_SPECTRAL_RELAX", 0.28),
    mean_relaxation=_env_float("MANN_CAL_MEAN_RELAX", 0.35),
    variance_relaxation=_env_float("MANN_CAL_VARIANCE_RELAX", 0.25),
    uw_relaxation=_env_float("MANN_CAL_UW_RELAX", 0.25),
    min_factor=_env_float("MANN_CAL_MIN_FACTOR", 0.65),
    max_factor=_env_float("MANN_CAL_MAX_FACTOR", 1.55),
