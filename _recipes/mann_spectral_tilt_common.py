...
It does not require windlespy to be importable.  It reads OpenFOAM probe files
and MannHybridTurb profile/spectra files directly.
"""
...
def update_profile_wong(current: pd.DataFrame, target: pd.DataFrame, downstream: pd.DataFrame, cfg: TiltConfig, cal_mask: np.ndarray) -> pd.DataFrame:
    out = current.copy()
    # map target/downstream to current z assumed already same length
    eps = 1e-12
    ...
    if cfg.update_uw_stress and cfg.profile_relax_uw > 0 and "uwStress" in current.columns:
        ...
        # Use a tiny target-based floor so a zero current value can start moving.
        mag_cur = np.maximum(np.abs(cur), 0.02*np.abs(tar))
        nxt = sign * mag_cur * fac
        out.loc[cal_mask, "uwStress"] = nxt[cal_mask]
    # optional small median smoothing in calibrated region only
    if cfg.smooth_window_z > 1 and np.any(cal_mask):
        cols = ["U", "Iu", "Iv", "Iw", "Lu", "Lv", "Lw", "uwStress"]
        sub = out.loc[cal_mask, cols].to_numpy(float)
        sm = smooth_profile_values(sub, cfg.smooth_window_z)
        out.loc[cal_mask, cols] = sm
    # Enforce local Reynolds-stress realizability after U/I updates.
...
def apply_spectral_tilt(S_current: np.ndarray, S_target: np.ndarray, S_down: np.ndarray, current_profile: pd.DataFrame, target_profile: pd.DataFrame, updated_profile: pd.DataFrame, freq: np.ndarray, cfg: TiltConfig, cal_mask: np.ndarray, resolved_fmin: Optional[np.ndarray] = None, resolved_fmax: Optional[np.ndarray] = None) -> Tuple[np.ndarray, Dict[str, np.ndarray]]:
    ...
    mu_t = spectral_centroid_logf(S_target, freq, f_update_min, f_update_max)
    mu_d = spectral_centroid_logf(S_down, freq, f_update_min, f_update_max)
    mu_c = spectral_centroid_logf(S_current, freq, f_update_min, f_update_max)
    metrics["mu_logf_target"] = mu_t
    metrics["mu_logf_downstream"] = mu_d
    metrics["mu_logf_current"] = mu_c
    lnf = np.log(np.maximum(freq, 1e-12))
    f_mask = (freq >= f_update_min) & (freq <= f_update_max)
    for comp in cfg.components:
        ...
        for j in range(S_new.shape[1]):
            ...
            # area update based on resolved-band variance deficit/surplus.
            cur_area = max(integrate_1d_between(freq, Sj, fmin_j, fmax_j), FLOOR)
            if cfg.normalize_to_target_variance:
                new_area = (target_profile[I_COL[comp]].iloc[j] * target_profile["U"].iloc[j])**2
            else:
            ...
            mode = cfg.mode.lower()
            if mode in {"moment", "hybrid"}:
                # Exponential tilting in log-frequency.  If downstream centroid is too low,
                # this boosts high f and suppresses low f, with smooth zero-area normalization later.
                dmu = float(mu_t[ci, j] - mu_d[ci, j])
                var_lnf = max(float(np.var(lnf[f_mask_j])), 1e-12)
                log_mult += cfg.moment_relax * dmu * (lnf - float(mu_c[ci, j])) / var_lnf
            if mode in {"bands", "hybrid"}:
                band_log = cfg.shape_relax * np.clip(np.log(np.maximum(E_t[ci, j, :], FLOOR) / np.maximum(E_d[ci, j, :], FLOOR)), -cfg.max_log_band_update, cfg.max_log_band_update)
                centers = np.sqrt(edges[:-1] * edges[1:])
                log_mult += np.interp(lnf, np.log(centers), band_log, left=band_log[0], right=band_log[-1])
            if mode == "twoband":
                # Split at target turnover frequency f ~ U/L.  L too high -> boost high band.
                Uj = max(float(target_profile["U"].iloc[j]), 1e-12)
                Lj = max(float(target_profile[L_COL[comp]].iloc[j]), 1e-12)
                fsplit = np.clip(Uj / Lj, f_update_min*1.2, f_update_max/1.2)
                low = (freq >= f_update_min) & (freq < fsplit)
                high = (freq >= fsplit) & (freq <= f_update_max)
...
def save_metrics_csv(metrics: Dict[str, np.ndarray], target: pd.DataFrame, cfg: TiltConfig, path: Path) -> None:
    ...
    for comp in COMPONENTS:
        ...
        for j, zj in enumerate(z):
            base = {"component": comp, "z": zj, "z_over_H": zj/cfg.building_height, "mu_logf_target": mu_t[ci,j], "mu_logf_downstream": mu_d[ci,j], "mu_logf_current": mu_c[ci,j]}
            for b in range(len(edges)-1):
                row = dict(base)
                row.update({"band": b, "f_low": edges[b], "f_high": edges[b+1], "E_target": E_t[ci,j,b], "E_downstream": E_d[ci,j,b], "E_current": E_c[ci,j,b], "log10_down_over_target": math.log10(max(E_d[ci,j,b], FLOOR)/max(E_t[ci,j,b], FLOOR))})
                rows.append(row)
...
def run_calibration(cfg: TiltConfig) -> int:
    ...
    updated_profile = update_profile_wong(current, target, downstream, cfg, cal_mask)
    S_updated, metrics = apply_spectral_tilt(S_current, S_target, S_down, current, target, updated_profile, freq, cfg, cal_mask, resolved_fmin, resolved_fmax)
    ...
    # Re-apply local realizability after final auto-spectrum/intensity normalization.
    ...
    print(f"Wrote updated profile, spectraProfile and uwCoSpectrumProfile to {profile_dir}", flush=True)
